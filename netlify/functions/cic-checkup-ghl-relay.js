...
const GHL_WEBHOOK_URL = process.env.CIC_CHECKUP_GHL_WEBHOOK_URL || "";
...
exports.handler = async (event) => {
  ...
  if (!GHL_WEBHOOK_URL) {
    return json(origin, 503, { ok: false, error: "Checkup delivery is not configured" });
  }
  ...
  try {
    ...
    console.error("CIC checkup relay failed:", error);
...
function normalizePayload(input) {
  return {
    ...
    intent: clean(input.intent || input.helpIntent || "Requested The Background Check Checkup PDF"),
    ...
    page: clean(input.page || input.page_url || input.pageUrl || "background-check-checkup"),
  };
}
